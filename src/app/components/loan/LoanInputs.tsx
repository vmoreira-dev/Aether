...
export default function LoanInputs() {
  ...
  // local draft state to allow free typing (future-proof)
  ...
  return (
    <div
      className="relative rounded-2xl border border-white/25 bg-white/[0.08] backdrop-blur-2xl shadow-[0_25px_80px_rgba(0,0,0,0.55)] px-8 pt-5 pb-7"
    >
      {/* inner chrome */}
          ...
          <div className="flex gap-2 mt-2">
            {TERM_PRESETS.map((m) => (
              <button
                ...
                className={
                  model.termMonths === m
                    ? "px-3 py-1 text-xs rounded-md border bg-white/15 border-white/35 transition-colors"
                    : "px-3 py-1 text-xs rounded-md border bg-white/5 border-white/20 hover:bg-white/10 transition-colors"
                }
              >
                {m} mo
              </button>
